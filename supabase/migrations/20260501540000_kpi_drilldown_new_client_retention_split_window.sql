-- =====================================================================
-- KPI drilldown fix: new_client_retention_6m / new_client_retention_12m
-- must use the split-window base cohort.
--
-- Problem
-- -------
-- private.debug_kpi_drilldown still sources its new_client_retention_*
-- rows from the old "base month = v_period_start + next 6/12 months
-- return window" model. The snapshot KPI was migrated to split-window
-- semantics in 20260501520000_kpi_new_client_retention_split_window_rule
-- so the card values no longer match the drilldown rows.
--
-- Observed symptom: the drilldown table returns the same ~5 April-2026
-- first-ever-seen rows for both the 6m and 12m KPIs, instead of the
-- 27 (6m) / 159 (12m) base-cohort rows reported by the card /
-- debug_kpi_new_client_retention_{6m,12m}.
--
-- Fix
-- ---
-- Rewrite only the new_client_retention_{6m,12m} branch of
-- private.debug_kpi_drilldown so it mirrors the split-window math
-- already used by the snapshot RPC and debug helpers:
--
--   * first half  = months 1..3 (6m) or 1..6 (12m) of the trailing
--                   6 / 12-month window ending at reporting month M.
--   * second half = months 4..6 (6m) or 7..12 (12m), MTD-clipped.
--   * base cohort = clients served at the requested scope in the first
--                   half AND whose first-ever appearance in the
--                   business is within the first half (NOT EXISTS
--                   sale_date < v_first_half_start).
--   * retained    = base-cohort clients seen in the second half
--                   (business-wide for staff scope, same-location for
--                   location scope, any for business scope).
--
-- Each drilldown row now represents exactly one base-cohort client:
--   row_type        = 'retained' | 'not_retained'
--   primary_label   = normalised name
--   secondary_label = 'Retained' | 'Not retained'
--   event_date      = first_visit_in_first_half (same as KPI anchor)
--   raw_payload adds
--     first_visit_in_window  = min(sale_date) across scope-agnostic
--                              non-internal rows in the full split
--                              window; because the cohort is
--                              restricted to first-ever-seen clients,
--                              this is effectively the client's
--                              first ever Oscar & Co visit.
--     last_visit_in_window   = max(sale_date) across the same universe.
--
-- Row count contract (validated below):
--   COUNT(*) FROM get_kpi_drilldown_live('new_client_retention_6m', ...)
--     == base_cohort_count FROM debug_kpi_new_client_retention_6m(...)
--   (same for 12m).
--
-- All other KPI branches (and the client_retention_* retention branch)
-- are preserved verbatim from 20260501530000.
-- =====================================================================
...
BEGIN
  ...
  -- ---------------- new_client_retention_6m / 12m (split-window) ----------------
  --
  -- Rewritten to mirror the snapshot KPI math from migration
  -- 20260501520000_kpi_new_client_retention_split_window_rule.sql.
  -- One row per base-cohort client.
  --
  -- base cohort = clients who were served at the requested scope in
  -- the first half AND whose first-ever appearance in the business is
  -- within the first half (NOT EXISTS sale_date < v_first_half_start).
  -- retained = base cohort ∩ clients seen in the second half at the
  -- scope-appropriate universe (business-wide for staff scope,
  -- same-location for location scope, any for business scope).
  IF v_kpi IN ('new_client_retention_6m', 'new_client_retention_12m') THEN
    ...
    first_half_stats AS (
      SELECT
        s.client_key,
        min(s.raw_name)  AS sample_raw,
        min(s.sale_date) AS first_visit_in_first_half
      FROM in_scope s
      WHERE s.sale_date BETWEEN v_first_half_start AND v_first_half_end
        AND (
          v_scope = 'business'
          OR (v_scope = 'location' AND s.loc_id   = v_loc_id)
          OR (v_scope = 'staff'    AND s.owner_id = v_staff_id)
        )
      GROUP BY s.client_key
    ),
    base_cohort AS (
      SELECT fhs.*
      FROM first_half_stats fhs
      WHERE NOT EXISTS (
        SELECT 1
        FROM public.v_sales_transactions_enriched e2
        WHERE e2.sale_date < v_first_half_start
          AND public.normalise_customer_name(e2.customer_name) = fhs.client_key
      )
    ),
    second_half_clients AS (
      SELECT DISTINCT s.client_key
      FROM in_scope s
      WHERE s.sale_date BETWEEN v_second_half_start AND v_second_half_end
        AND (
          v_scope = 'business'
    ...
    SELECT
      ...
      CASE WHEN r.client_key IS NOT NULL THEN 'Retained' ELSE 'Not retained' END::text
                                                                            AS secondary_label,
      (CASE WHEN r.client_key IS NOT NULL THEN 1 ELSE 0 END)::numeric       AS metric_value,
      NULL::numeric                                                         AS metric_value_2,
      b.first_visit_in_first_half                                           AS event_date,
      NULL::uuid                                                            AS location_id,
      NULL::uuid                                                            AS staff_member_id,
      jsonb_build_object(
        'normalised_name',        b.client_key,
        'sample_raw_name',        b.sample_raw,
        'first_visit_in_base',    b.first_visit_in_first_half,
        'retained',               (r.client_key IS NOT NULL),
        'first_half_start',       v_first_half_start,
        'first_half_end',         v_first_half_end,
        'second_half_start',      v_second_half_start,
    ...
    FROM base_cohort b
    LEFT JOIN second_half_clients r  ON r.client_key  = b.client_key
    LEFT JOIN all_visits          av ON av.client_key = b.client_key
    ORDER BY (r.client_key IS NOT NULL) DESC, b.client_key;
    RETURN;
  END IF;
...
'DEBUG / VALIDATION ONLY. Raw-row drilldown for each supported KPI. Mirrors the filters / windows / scope rules of the matching live KPI RPC. Retention branches (client_retention_* and new_client_retention_*) additionally expose first_visit_in_window / last_visit_in_window in raw_payload so the UI can render first/last visit date columns. new_client_retention_{6m,12m} now follow the split-window base-cohort rule (mirrors debug_kpi_new_client_retention_{6m,12m}); one row per base-cohort client. Not exposed via PostgREST. Drop when the v1 KPI validation phase is over.';
