-- Location-scoped replacement for Sales Daily Sheets: before applying a batch, remove prior
-- SalesDailySheets payroll imports for the same location only (transactions, raw rows, batches).
-- Full multi-location wipe remains delete_all_sales_daily_sheets_import_data().
