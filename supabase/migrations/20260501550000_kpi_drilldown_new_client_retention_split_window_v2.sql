-- =====================================================================
-- KPI drilldown fix, v2: force-reapply the split-window base cohort
-- for new_client_retention_{6m,12m} inside private.debug_kpi_drilldown.
--
-- Why this migration exists
-- -------------------------
-- Migration 20260501540000 replaced private.debug_kpi_drilldown with
-- the split-window cohort body. Validation against the live DB
-- (Jarod, April 2026) still returns 1 drilldown row while the snapshot
-- debug helper returns base_cohort_count = 57 / 224 for 6m / 12m.
-- That is only explainable if 20260501540000's body never reached the
-- database. The SELECT is per-row and cannot collapse to 1.
--
-- This migration ships the same fix under a new timestamp so
-- `supabase db push` definitely picks it up, and tags the COMMENT with
-- a version marker so the user can read it back to confirm which body
-- is active before re-running the row-count validation.
--
-- Behaviour (unchanged from the intended 20260501540000 body)
-- -----------------------------------------------------------
...
--
-- Each drilldown row = one base-cohort client:
--   row_type        = 'retained' | 'not_retained'
--   primary_label   = normalised name
--   secondary_label = 'Retained' | 'Not retained'
--   event_date      = first_visit_in_first_half
--   raw_payload.first_visit_in_window / last_visit_in_window added.
--
-- Row count contract
-- ------------------
--   COUNT(*) FROM debug_kpi_drilldown('new_client_retention_6m',  ...)
--     == base_cohort_count FROM debug_kpi_new_client_retention_6m(...)
--   COUNT(*) FROM debug_kpi_drilldown('new_client_retention_12m', ...)
--     == base_cohort_count FROM debug_kpi_new_client_retention_12m(...)
--
-- All other KPI branches (revenue / guests_per_month /
-- new_clients_per_month / average_client_spend / client_frequency /
-- client_retention_{6m,12m} / assistant_utilisation_ratio /
-- stylist_profitability) are preserved verbatim from 20260501530000.
-- =====================================================================
...
BEGIN
  ...
  --
  -- One row per base-cohort client. Math mirrors
  -- public.get_kpi_new_client_retention_{6m,12m}_live and
  -- private.debug_kpi_new_client_retention_{6m,12m} from
  -- 20260501520000. No aggregation at the top-level SELECT; the row
  -- count equals |base_cohort|.
...
'DEBUG / VALIDATION ONLY. drilldown_version=split-window-v2 (20260501550000). Raw-row drilldown for each supported KPI. Retention branches (client_retention_* and new_client_retention_*) expose first_visit_in_window / last_visit_in_window in raw_payload. new_client_retention_{6m,12m} follows the split-window base-cohort rule (mirrors debug_kpi_new_client_retention_{6m,12m}); one row per base-cohort client. Not exposed via PostgREST. Drop when the v1 KPI validation phase is over.';
