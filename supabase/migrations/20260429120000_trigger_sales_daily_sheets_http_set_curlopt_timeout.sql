-- pgsql-http applies a hardcoded 5000 ms CURLOPT_TIMEOUT_MS on each request handle, then overlays
-- user options from internal curlopt_val. set_config('http.curlopt_timeout_ms', ...) does not always
-- update the slot the extension reads before http_post in practice; use extensions.http_set_curlopt
-- (see pgsql-http http_set_curlopt / CURLOPT_TIMEOUT_MS).
