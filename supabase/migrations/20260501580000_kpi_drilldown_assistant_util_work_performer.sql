-- =====================================================================
-- KPI drilldown: expose work-performer on assistant_utilisation_ratio
--
-- Why this migration exists
-- -------------------------
-- The assistant_utilisation_ratio drilldown currently returns only the
-- commission owner (secondary_label) and not the staff member who
-- physically performed the work. The frontend needs both columns
-- visible side-by-side in the Assistant Utilisation underlying rows
-- table ("Owner" and "Work Performed By").
--
-- Change
-- ------
-- Only the assistant_utilisation_ratio branch of
-- private.debug_kpi_drilldown is modified: its raw_payload
-- jsonb now also carries staff_work_display_name / staff_work_full_name
-- / staff_work_name sourced from public.v_sales_transactions_enriched.
-- The row shape, KPI math, filters, sort, secondary_label, and every
-- other branch (revenue / guests_per_month / new_clients_per_month /
...
-- verbatim from 20260501550000.
--
-- Frontend consumption
-- --------------------
--   Owner              = secondary_label (commission_owner_candidate_name)
--   Work Performed By  = raw_payload.staff_work_display_name
--                          || staff_work_full_name
--                          || staff_work_name  (fallback chain)
-- =====================================================================
...
BEGIN
  ...
  -- MODIFIED in 20260501580000: raw_payload now carries
  -- staff_work_display_name / staff_work_full_name / staff_work_name so
  -- the frontend can render "Work Performed By" as a separate column
  -- alongside "Owner" (commission_owner_candidate_name). No KPI math
  -- changes; secondary_label still exposes the owner.
...
'DEBUG / VALIDATION ONLY. drilldown_version=split-window-v2 (20260501550000) + assistant-work-performer (20260501580000). Raw-row drilldown for each supported KPI. Retention branches (client_retention_* and new_client_retention_*) expose first_visit_in_window / last_visit_in_window in raw_payload. new_client_retention_{6m,12m} follows the split-window base-cohort rule (mirrors debug_kpi_new_client_retention_{6m,12m}); one row per base-cohort client. assistant_utilisation_ratio raw_payload now also carries staff_work_{display_name,full_name,name,id,primary_role} so the UI can render Owner vs Work Performed By as separate columns. Not exposed via PostgREST. Drop when the v1 KPI validation phase is over.';
