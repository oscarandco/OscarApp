-- =====================================================================
-- KPI drilldown: per-visit invoice list on guest-based KPI rows
--
-- Adds `visit_invoices` (JSON array of {sale_date, location_id, invoice},
-- ordered by sale_date then invoice) to raw_payload for:
--   guests_per_month, new_clients_per_month, average_client_spend
--
-- Row-level metric_value / metric_value_2 / event_date / sort order are
-- unchanged — display-only enrichment for KPI drilldown UI.
--
-- Full function body copied from 20260501590000_kpi_drilldown_revenue_invoice_product.sql
-- with the three guest branches extended as above.
-- =====================================================================
  ...
  IF v_kpi = 'guests_per_month' THEN
    ...
    visit_list AS (
      SELECT
        ...
        s.sale_date,
        s.loc_id,
        min(nullif(btrim(COALESCE(s.invoice::text, '')), '')) AS invoice
      FROM in_scope s
      GROUP BY s.client_key, s.sale_date, s.loc_id
    ),
    visit_invoices_agg AS (
      SELECT
        vl.client_key,
        COALESCE(
          jsonb_agg(
            jsonb_build_object(
              'sale_date', vl.sale_date::text,
              'location_id', vl.loc_id::text,
              'invoice', vl.invoice
            )
            ORDER BY vl.sale_date::text, vl.invoice
          ) FILTER (WHERE vl.invoice IS NOT NULL),
          '[]'::jsonb
        ) AS visit_invoices
      FROM visit_list vl
      GROUP BY vl.client_key
    )
    SELECT
      ...
      jsonb_build_object(
        ...
        'visit_invoices',  COALESCE(vi.visit_invoices, '[]'::jsonb)
      )                                                             AS raw_payload
    FROM agg a
    LEFT JOIN visit_invoices_agg vi ON vi.client_key = a.client_key
  ...
  IF v_kpi = 'new_clients_per_month' THEN
    ...
    visit_list AS (
      SELECT
        ...
        min(nullif(btrim(COALESCE(s.invoice::text, '')), '')) AS invoice
      FROM in_scope s
      INNER JOIN new_clients nc ON nc.client_key = s.client_key
      GROUP BY s.client_key, s.sale_date, s.loc_id
    ),
    visit_invoices_agg AS (
      SELECT
    ...
    FROM new_clients nc
    LEFT JOIN visit_invoices_agg vi ON vi.client_key = nc.client_key
