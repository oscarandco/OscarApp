-- FTE-normalise comparison values for revenue, guests_per_month, and
-- new_clients_per_month so cohort high/avg/current and is_highest /
-- is_above_average match the card's displayed normalised numbers for
-- stylists with 0 < fte < 1. average_client_spend and
-- assistant_utilisation_ratio stay on raw cohort math (rates / ratios).
...
'Live stylist comparison for staff/self KPI cards. Set-based slice of v_sales_transactions_enriched for cohort; revenue / guests / new_clients use FTE-adjusted comparison values (raw/fte when 0<fte<1) so badges align with normalised cards; average_client_spend and assistant_utilisation_ratio use raw cohort ratios.';
