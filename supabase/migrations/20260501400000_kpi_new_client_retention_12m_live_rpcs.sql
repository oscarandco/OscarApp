-- =====================================================================
-- KPI live RPC + debug mirror: new_client_retention_12m
--
-- Locked definition (docs/KPI App Architecture.md):
--   new_client_retention_12m = true acquisition-cohort retention.
--
...
--                  SCOPE within the next 12 calendar months after the
--                  base month.
...
--     (business-wide NOT EXISTS against prior history). This matches
--     the rule used by get_kpi_new_clients_per_month_live /
--     get_kpi_new_client_retention_6m_live and does NOT filter the
--     historical check by internal/scope - any prior appearance
--     disqualifies the client from being "new".
--
...
--     return_window_end_full = (p + interval '13 months' - interval '1 day')::date  -- exactly 12 calendar months after base_month_end_full
--     return_window_end_obs  = LEAST(return_window_end_full, current_date)
--
--   Example p = 2024-06-01:
--     base_month    = [2024-06-01, 2024-06-30]
--     return_window = [2024-07-01, 2025-06-30]   (12 months: Jul 2024..Jun 2025)
--
...
--     new_clients_per_month / new_client_retention_6m. The cohort will
--     keep growing through the remainder of the month.
...
--     and the immediately-preceding month until day 1 of the month
--     after), retained = 0 by construction. The value then reflects
--     "retention observed so far" and will climb monotonically as the
--     return window matures.
...
--
--   Reporting month anchors the START of the retention window (i.e. it
--   is the acquisition/base month, NOT the end of the return window).
--   The return window runs strictly AFTER the base month and is 12 full
...
--
-- Performance caveat: same as new_clients_per_month /
-- new_client_retention_6m - the "first ever seen" check is a
-- business-wide NOT EXISTS over v_sales_transactions_enriched against
-- normalise_customer_name. At current data volume this runs well under
-- a second. If the historical dataset grows by ~10x, the right additive
-- next step is an expression index on
-- public.sales_transactions(normalise_customer_name(customer_name))
-- - intentionally NOT added here to keep this migration purely additive.
-- =====================================================================


-- ---------------------------------------------------------------------
-- 1. public.get_kpi_new_client_retention_12m_live  (auth-enforced)
-- ---------------------------------------------------------------------
...
'Live 12-month new-client acquisition-cohort retention KPI. Cohort = clients whose first-ever business appearance is in the base/reporting month AND who were seen at the requested scope (business / same location / same stylist). Retained = cohort clients who return at the same scope in the 12 calendar months AFTER the base month. Return window is MTD-clipped; maturity is encoded in the source text and exposed explicitly by the debug helper (is_return_window_complete). Live RPC preserves the locked 12-column KPI return shape. "First ever seen" check is business-wide per the locked new-client rule.';


-- =====================================================================
-- 2. private.debug_kpi_new_client_retention_12m  (validation only)
-- =====================================================================

CREATE OR REPLACE FUNCTION private.debug_kpi_new_client_retention_12m(
  p_period_start    date,
  p_scope           text DEFAULT 'business',
  p_location_id     uuid DEFAULT NULL,
  p_staff_member_id uuid DEFAULT NULL
...
'DEBUG / VALIDATION ONLY. Mirror of public.get_kpi_new_client_retention_12m_live without the auth wrapper. Adds base/return window boundaries, cohort_count, retained_count, in_period_guest_count (= guests_per_month-style count at same scope) so the cohort can be reconciled against new_clients_per_month. Drop when validation is complete.';
