-- =====================================================================
-- KPI live RPC + debug mirror: client_retention_6m
--
-- Locked definition (docs/KPI App Architecture.md):
--   client_retention_6m = rolling 6-month split-window retention.
--
--   For a reporting month:
--     * base cohort = distinct clients seen in months 1-3 of the
--       trailing 6-month window.
--     * retained    = those cohort clients seen again in months 4-6
--       of the same window.
--     * value       = retained / base cohort, NULL when base cohort = 0.
--
-- Source and conventions carried over unchanged from client_frequency:
...
--
-- Window semantics (reporting month anchors month 6 of 6)
-- -------------------------------------------------------
--   Let p = p_period_start (first-of-month).
--
--     first_half_start  = (p - interval '5 months')::date
--     first_half_end    = ((p - interval '2 months')::date - 1)
--     second_half_start = (p - interval '2 months')::date
--     second_half_end   = LEAST(period_end, current_date)   -- mtd_through
--
--   Example p = 2026-04-01:
--     first_half  = [2025-11-01, 2026-01-31]   (months 1-3: Nov, Dec, Jan)
--     second_half = [2026-02-01, 2026-04-30]   (months 4-6: Feb, Mar, Apr)
--
--   Closed-month behaviour:
--     second_half_end = period_end (complete 3-month return window).
--
--   Current-open-month behaviour:
--     second_half_end = current_date (MTD-clipped). The value is
--     "retention as of today MTD" and trends up through the month
--     as base-cohort clients return. This matches the MTD-clip
--     convention already locked for revenue / guests / client_frequency
--     and every other live KPI. The base cohort window is unaffected
--     by MTD because it always ends ≥ 2 months before the reporting
--     month begins.
--
...
-- ---------------------------------------------------------------------
-- 1. public.get_kpi_client_retention_6m_live  (auth-enforced)
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  v_source := format(
    'v_sales_transactions_enriched: retained / base cohort; base=[%s,%s] (months 1-3); return=[%s,%s] (months 4-6); non-internal; scope of return = %s',
...
ALTER FUNCTION public.get_kpi_client_retention_6m_live(date, text, uuid, uuid) OWNER TO postgres;
REVOKE ALL ON FUNCTION public.get_kpi_client_retention_6m_live(date, text, uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_kpi_client_retention_6m_live(date, text, uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_kpi_client_retention_6m_live(date, text, uuid, uuid) TO service_role;

COMMENT ON FUNCTION public.get_kpi_client_retention_6m_live(date, text, uuid, uuid) IS
'Live 6-month split-window client retention KPI. Base cohort = distinct normalise_customer_name in months 1-3 of the trailing 6-month window ending with the reporting month. Retained = those cohort clients seen again in months 4-6 at the same scope (business / same location / same stylist). Current open month is MTD-clipped for the second-half end date only.';


-- =====================================================================
-- 2. private.debug_kpi_client_retention_6m  (validation only)
--
...
ALTER FUNCTION private.debug_kpi_client_retention_6m(date, text, uuid, uuid) OWNER TO postgres;
REVOKE ALL ON FUNCTION private.debug_kpi_client_retention_6m(date, text, uuid, uuid) FROM PUBLIC;

COMMENT ON FUNCTION private.debug_kpi_client_retention_6m(date, text, uuid, uuid) IS
'DEBUG / VALIDATION ONLY. Mirror of public.get_kpi_client_retention_6m_live without the auth wrapper. Adds explicit first_half / second_half boundaries and base_cohort_count / retained_count / row_count_in_window for sanity-checking. Drop when validation is complete.';
