-- =====================================================================
-- client_frequency KPI: fix remaining "visit_count is ambiguous" error.
--
-- Root cause
-- ----------
-- private.debug_kpi_client_frequency declares
--   RETURNS TABLE (..., visit_count bigint, client_count bigint,
--                       row_count_in_window bigint)
-- which makes those three identifiers implicit PL/pgSQL variables
-- inside the function body. The `agg` CTE from migration
-- 20260501350000 also selected columns named `visit_count`,
-- `client_count`, and `row_count_in_window`, so
--
--   SELECT visit_count, client_count, row_count_in_window
--   INTO   v_visits, v_clients, v_rows
--   FROM   agg;
--
-- was ambiguous and failed with:
--   ERROR: column reference "visit_count" is ambiguous
--
-- Fix
-- ---
-- Rename the `agg` CTE columns to neutral aliases (`v_count`,
-- `c_count`, `r_count`) that cannot collide with any RETURNS TABLE
-- output column. The `INTO` targets and the `RETURN QUERY` output
-- columns stay exactly as they were. No KPI semantics, scope rules,
-- visit-unit definition, auth, or return shape change.
--
-- public.get_kpi_client_frequency_live does not expose visit_count /
-- client_count / row_count_in_window, so it was not affected by this
-- specific error. It is re-emitted unchanged from migration
-- 20260501350000 for completeness, to leave both functions in a
...
--    Re-emitted unchanged from 20260501350000 — no functional delta.
-- ---------------------------------------------------------------------
...
--    CHANGED: `agg` CTE columns renamed to v_count / c_count / r_count
--    to avoid collision with the RETURNS TABLE output variables
--    visit_count / client_count / row_count_in_window.
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  agg AS (
    SELECT
      (SELECT COUNT(*)                   FROM visits)   AS v_count,
      (SELECT COUNT(DISTINCT client_key) FROM visits)   AS c_count,
      (SELECT COUNT(*)                   FROM in_scope) AS r_count
  )
  SELECT v_count, c_count, r_count
  INTO v_visits, v_clients, v_rows
  FROM agg;
