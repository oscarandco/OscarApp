-- =====================================================================
-- KPI drilldown: expose invoice + product on sales-line branches
--
-- Why this migration exists
-- -------------------------
-- The KPI page's underlying-rows table for sales-line KPIs (revenue,
-- assistant_utilisation_ratio) now shows Invoice Number and Product as
-- dedicated columns and lets the user open an invoice detail popup.
-- That needs both the invoice text and the product/service description
-- carried on each drilldown row, and for assistant-utilisation rows
-- alongside the already-present work-performer fields.
--
-- Change
-- ------
-- Only the `revenue` and `assistant_utilisation_ratio` branches of
-- private.debug_kpi_drilldown are modified. Each of those raw_payload
-- jsonb objects now also carries:
--   * invoice
--   * sale_datetime
--   * product_service_name
--   * product_type_actual
--   * staff_work_id, staff_work_name, staff_work_display_name,
--     staff_work_full_name, staff_work_primary_role
--     (already on assistant branch since 20260501580000, now added to
--      revenue branch too — small, safe, cheap to include, and useful
--      as Work Performed By secondary context inside the invoice modal)
--
-- All other branches (guests_per_month / new_clients_per_month /
-- average_client_spend / client_frequency / client_retention_{6m,12m} /
-- new_client_retention_{6m,12m} / stylist_profitability) are preserved
-- verbatim from 20260501580000. The row shape, KPI math, filters,
-- sort, secondary_label, and every metric value are unchanged on every
-- branch.
-- =====================================================================
...
BEGIN
  ...
  -- ---------------- revenue ----------------
  -- MODIFIED in 20260501590000: raw_payload now carries invoice,
  -- sale_datetime, product_service_name, product_type_actual, and the
  -- staff_work_* chain. No KPI math changes.
  IF v_kpi = 'revenue' THEN
    ...
    SELECT
      ...
      jsonb_build_object(
        ...
        'sale_datetime',                    e.sale_datetime,
        'month_start',                      e.month_start,
        'invoice',                          e.invoice,
        'product_service_name',             e.product_service_name,
        'product_type_actual',              e.product_type_actual,
        ...
        'staff_work_id',                    e.staff_work_id,
        'staff_work_name',                  e.staff_work_name,
        'staff_work_display_name',          e.staff_work_display_name,
        'staff_work_full_name',             e.staff_work_full_name,
        'staff_work_primary_role',          e.staff_work_primary_role
      )                                                                          AS raw_payload
  ...
  IF v_kpi IN ('new_client_retention_6m', 'new_client_retention_12m') THEN
    ...
    SELECT
      ...
      jsonb_build_object(
        ...
        'first_visit_in_window',  av.first_visit_in_window,
        'last_visit_in_window',   av.last_visit_in_window,
        'drilldown_version',      'split-window-v2 (20260501550000)'
      )                                                                     AS raw_payload
    FROM base_cohort b
  ...
  -- MODIFIED in 20260501590000: raw_payload now also carries
  -- invoice / sale_datetime / product_service_name / product_type_actual
  -- alongside the work-performer fields added in 20260501580000.
...
'DEBUG / VALIDATION ONLY. drilldown_version=split-window-v2 (20260501550000) + assistant-work-performer (20260501580000) + sales-line-invoice-product (20260501590000). Raw-row drilldown for each supported KPI. Retention branches (client_retention_* and new_client_retention_*) expose first_visit_in_window / last_visit_in_window in raw_payload. new_client_retention_{6m,12m} follows the split-window base-cohort rule (mirrors debug_kpi_new_client_retention_{6m,12m}); one row per base-cohort client. Sales-line branches (revenue / assistant_utilisation_ratio) carry invoice / sale_datetime / product_service_name / product_type_actual + staff_work_{display_name,full_name,name,id,primary_role} so the UI can render Invoice Number / Product / Work Performed By columns and open an invoice-detail modal. Not exposed via PostgREST. Drop when the v1 KPI validation phase is over.';
