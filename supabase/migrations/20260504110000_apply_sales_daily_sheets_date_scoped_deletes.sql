-- Scope SalesDailySheets apply deletes to the staged file's sale-date window
-- (location + min/max from staged rows) so a 12-month re-import does not
-- purge years of history and hit statement_timeout.
...
COMMENT ON FUNCTION public.apply_sales_daily_sheets_to_payroll(uuid) IS
  ...
  'Deletes existing SalesDailySheets rows only for the batch location and the staged file '
  'sale_date range (min/max from staged rows, same date logic as the INSERT). '
  'Drops empty sales_import_batches headers (excluding the active payroll batch) after deletes.';
...
-- Supporting indexes for date-scoped deletes (IF NOT EXISTS: baseline may add some later).
CREATE INDEX IF NOT EXISTS idx_sales_transactions_location_id_sale_date
  ON public.sales_transactions (location_id, sale_date);

CREATE INDEX IF NOT EXISTS idx_sales_transactions_import_batch_id_sale_date
  ON public.sales_transactions (import_batch_id, sale_date);

CREATE INDEX IF NOT EXISTS idx_raw_sales_import_rows_import_batch_id_sale_datetime
  ON public.raw_sales_import_rows (import_batch_id, sale_datetime);

-- idx_sales_import_batches_source_location (source_name, location_id) already from 20260501610000.
