-- Stored role `assistant`: same payroll row visibility as `self` (Stylist) in access-scoped views.
-- UI label: Assistant. Not elevated (not admin/manager).
-- Legacy `self` is migrated to `stylist` in 20260430210000_access_role_stylist_replaces_self.sql.
...
ALTER TABLE public.staff_member_user_access
  ...
  CHECK (
    access_role = ANY (
      ARRAY['self'::text, 'assistant'::text, 'manager'::text, 'admin'::text]
    )
  );
 ...
 SELECT a.user_id,
   ...
   FROM v_admin_payroll_lines_weekly l
     JOIN staff_member_user_access a ON a.is_active = true
       AND (
         (a.access_role = ANY (ARRAY['self'::text, 'assistant'::text])
