-- Fix storage cleanup (replaces 20260424120000 direct DELETE FROM storage.objects):
-- after merge, call the Edge Function with cleanup_storage_only so it uses supabase.storage.remove().
-- Import/business logic unchanged apart from cleanup mechanism.
...
DECLARE
  ...
  v_sql jsonb;
  v_out jsonb;
  v_storage_cleanup_ok boolean;
  v_cleanup_status int;
  v_cleanup_content text;
  v_cleanup_body jsonb;
BEGIN
  ...
  IF v_edge_url IS NOT NULL AND v_secret IS NOT NULL
    ...
    BEGIN
      ...
      v_storage_cleanup_ok := true;
      BEGIN
        SELECT status, content::text
        INTO v_cleanup_status, v_cleanup_content
        FROM extensions.http_post(
          v_edge_url,
          jsonb_build_object(
            'batch_id', v_batch_id,
            'storage_path', v_path,
            'internal_secret', v_secret,
            'cleanup_storage_only', true
          )::text,
          'application/json'
        );

        IF v_cleanup_status IS NULL OR v_cleanup_status < 200 OR v_cleanup_status >= 300 THEN
          v_storage_cleanup_ok := false;
          RAISE WARNING 'trigger_sales_daily_sheets_import: storage cleanup HTTP %: %',
            v_cleanup_status, left(coalesce(v_cleanup_content, ''), 2000);
        ELSE
          v_cleanup_body := v_cleanup_content::jsonb;
          IF NOT (
            (v_cleanup_body @> '{"ok": true}'::jsonb)
            OR (lower(trim(coalesce(v_cleanup_body->>'ok', ''))) IN ('true', 't', '1'))
          ) THEN
            v_storage_cleanup_ok := false;
            RAISE WARNING 'trigger_sales_daily_sheets_import: storage cleanup failed: %',
              left(v_cleanup_content, 2000);
          END IF;
        END IF;
      EXCEPTION
        WHEN OTHERS THEN
          v_storage_cleanup_ok := false;
          RAISE WARNING 'trigger_sales_daily_sheets_import: storage cleanup exception: %', SQLERRM;
      END;
      ...
      SET
        ...
        message = CASE
          WHEN v_storage_cleanup_ok THEN 'Import completed'
          ELSE 'Import completed; temporary CSV could not be removed from storage (check DB logs)'
        END,
        error_message = NULL
      WHERE b.id = v_batch_id;
      ...
      RETURN jsonb_build_object(
        ...
        CASE
          WHEN v_storage_cleanup_ok THEN 'Import completed'
          ELSE 'Import completed (temporary file left in storage; cleanup logged)'
        END,
        'batch_id', v_batch_id,
        'storage_path', v_path,
        'rows_staged', v_rows_staged,
        ...
        'status', 'completed',
        'error_message', NULL,
        'storage_removed', v_storage_cleanup_ok
      );
