...
export function aggregateTableAByStaff(lines: AdminPayrollLineRow[]): TableARow[] {
  const map = new Map<
    string,
    { profProd: number; retailProd: number; services: number; staffPaidId: string | null }
  >()
  ...
  for (const row of lines) {
    ...
    if (!map.has(key)) {
      map.set(key, { profProd: 0, retailProd: 0, services: 0, staffPaidId: null })
    }
    const b = map.get(key)!
    ensureStaffId(b, row)
...
export function aggregateTableBByStaff(lines: AdminPayrollLineRow[]): TableBRow[] {
  const map = new Map<string, { commProducts: number; commServices: number; staffPaidId: string | null }>()
  ...
  for (const row of lines) {
    ...
    if (!map.has(key)) {
      map.set(key, { commProducts: 0, commServices: 0, staffPaidId: null })
    }
    const b = map.get(key)!
    ensureStaffId(b, row)
